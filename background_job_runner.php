...
//             Log::channel('background_jobs_errors')->error("Job $className::$methodName failed after $maxRetries attempts.");
//             exit(1);
//         }
//     }
...
// Function to check if the job is allowed
function isAllowedJob(string $className, string $methodName, array $allowedJobs): bool
{
    return isset($allowedJobs[$className]) && in_array($methodName, $allowedJobs[$className]);
}
...
do {
    ...
    try {
        ...
        // Update job retry count and retry in case of failure
        DB::table('jobs')->where('id', $jobId)->update([
            'retry_count' => $attempts,
            'updated_at' => now(),
        ]);
